...
def driveDiagonalDistance(setpoint,duration):
    ...
    while(brain.timer_time(SECONDS)<duration):
        ...
        currentXLocation = location.position(X,MM)
        currentYLocation = location.position(Y,MM)
        if(currentXLocation < setpoint - tolerance and currentYLocation < setpoint - tolerance):
            drivetrain.drive(FORWARD)
        elif (currentXLocation > setpoint + tolerance and currentYLocation < setpoint + tolerance):
            drivetrain.drive(REVERSE)
        #VEXCode VR requires that we have a small pause in any loop we run.    
        wait(1,MSEC)
    drivetrain.stop()
